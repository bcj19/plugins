...
function doCompare(key, first, second) {
  var compareType = '';
...
function doMerge(controlHost, challengeHost, controlParams, challengeParams) {
  var output = {};
  ...
  for (var key in controlHost) {
    var ctrl = (controlHost[key] && controlHost[key] != null) ? controlHost[key] : '',
      ...
      compare = doCompare(key, ctrl, chal);
...
for (var i = 0, maxi = controlCalls.length; i < maxi; i++) {
  ...
  controlHostInfo = getHostInfo(controlHostInfo);
  challengeHostInfo = getHostInfo(challengeHostInfo);

  var controlParams = getParamInfo(controlSearch),
    challengeParams = getParamInfo(challengeSearch);

  output.push(doMerge(controlHostInfo, challengeHostInfo, controlParams, challengeParams));
}
